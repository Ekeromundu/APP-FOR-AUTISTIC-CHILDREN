        ...
        public void execute() {
            ...
            executor.execute(() -> {
                ...
                try {
                    ...
                    jsonBody.put("q", texto); // text to translate
                    jsonBody.put("source", mSource); // source language code, e.g. "en"
                    jsonBody.put("target", mTarget); // target language code, e.g. "es"
                    jsonBody.put("format", "text"); // format can be "text" or "html"
                    ...
                    JsonObjectRequest myRequest = new JsonObjectRequest(
                            ...
                            response -> {
                                try {
                                    ...
                                    anInterface.onTextoTraducido(true);
                                    Log.d("LibreTranslateResponse", translatedText);
                                } catch (Exception e) {
                                    Log.e("LibreTranslateError", e.toString());
                                }
                            },
                            error -> {
                                Log.e("LibreTranslateError", error.toString());
                                CustomToast.getInstance(mContext).mostrarFrase(mContext.getString(R.string.str_error_translateGoogle));
                                new Handler().postDelayed(() -> anInterface.onTextoTraducido(true), 4000);
                            }
                    ) {
                        ...
                        public Map<String, String> getHeaders() {
                            HashMap<String, String> headers = new HashMap<>();
                            headers.put("Content-Type", "application/json; charset=utf-8");
                            // No Authorization header needed for LibreTranslate public endpoint
                            return headers;
                        }
                    };
